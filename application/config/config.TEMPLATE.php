...
// file : /php-login/application/config.config.TEMPLATE.php
// id : # 20140627.1821
// note : This file is modified to recognize MySQL/PostgreSQL/SQLite.
// note : Use searchword '(PdoDbMore)' to find all database switch
...
//  settings in the middle of the file. [seq # 20140627.1832]

//const DB_SWITCH = PdoDbMore::MySQL;
//const DB_SWITCH = PdoDbMore::PgSQL;
const DB_SWITCH = PdoDbMore::SQLite;
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
 ...
 */
// IF YOU CHANGE THIS, ADJUST AS WELL RewriteBase in .htaccess!
define('URL', 'http://localhost/php-login-xdb/'); // (MYSETTING)
...
*/
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// Database settings for various databases (PdoDbMore) [seq # 20140627.1831] (MYSETTING)
if (DB_SWITCH == PdoDbMore::MySQL)
{
   define('DB_TYPE', 'mysql');
...
{
   die('Database switch not selected correctly.');
}
define('DB_FILE_ABS', realpath(DB_FILE));
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
define("PHPMAILER_DEBUG_MODE", 0); // (MYSETTING)
// use SMTP or basic mail() ? SMTP is strongly recommended
define("EMAIL_USE_SMTP", false); // (MYSETTING)
// name of your host
define("EMAIL_SMTP_HOST", 'yourhost'); // (MYSETTING)
// leave this true until your SMTP can be used without login
define("EMAIL_SMTP_AUTH", true); // (MYSETTING)
// SMTP provider username
define("EMAIL_SMTP_USERNAME", 'yourusername'); // (MYSETTING)
// SMTP provider password
define("EMAIL_SMTP_PASSWORD", 'yourpassword'); // (MYSETTING)
// SMTP provider port
define("EMAIL_SMTP_PORT", 465);
...
define("EMAIL_PASSWORD_RESET_FROM_EMAIL", "no-reply@example.com"); // (MYSETTING)
define("EMAIL_PASSWORD_RESET_FROM_NAME", "My Project");
define("EMAIL_PASSWORD_RESET_SUBJECT", "Password reset for PROJECT XY");
define("EMAIL_PASSWORD_RESET_CONTENT", "Please click on this link to reset your password: ");

define("EMAIL_VERIFICATION_URL", URL . "login/verify");
define("EMAIL_VERIFICATION_FROM_EMAIL", "no-reply@example.com"); // (MYSETTING)
define("EMAIL_VERIFICATION_FROM_NAME", "My Project");
define("EMAIL_VERIFICATION_SUBJECT", "Account activation for PROJECT XY");
define("EMAIL_VERIFICATION_CONTENT", "Please click on this link to activate your account: ");
